...
async function getShareUrl(str) {
...
window.addEventListener('load', function() {
  if (inIframe()) {
    ...
    window.addEventListener('keydown', ev => {
      if (["Space", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(ev.key)) {
        ev.preventDefault();
      }
    });
...
function pokiSendLevelData() {
  navigator.sendBeacon('https://leveldata.poki.io/oddbotout-7BB6514C1DF0568A', 'c7eea996-4017-4e5a-8b19-7acc5cc56695');
}
...
function handleFileDrop(path) {
  ...
  const url = URL.createObjectURL(new Blob([content.buffer], {
    type: Browser.getMimetype(path)
  }));
  scanImage(url, true);
}
...
function uint8ArrayToBase64(buf) {
  return btoa(Array(buf.length).fill('').map((_, i) => String.fromCharCode(buf[i])).join(''));
}
...
function tryReadUrlLevel() {
  let levelStr = parseUrlArgument("l");
  if (levelStr) {
    let decoded = decodeURIComponent(levelStr);
    let arr = base64ToUint8Array(decoded);
...
function showShareDialog(path) {
  ...
  const cardUrl = URL.createObjectURL(new Blob([cardBuf.buffer], {
    type: Browser.getMimetype(cardPath)
  }));
  let buf = readLocalFile(path);
  let b64 = uint8ArrayToBase64(buf);
  let encoded = encodeURIComponent(b64);
  let w = Math.min(window.innerWidth, 1000);
  let maxCanvasW = w * 0.4;
  getShareUrl(encoded).then((url) => {
    let html = `
